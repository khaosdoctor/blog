...
heroImage: "./boas-praticas-ts-semana-ts-3/Artboard-1-2.png"
draft: false
visibility: "members"
...
![](./boas-praticas-ts-semana-ts-3/image-50.png)
...
![](./boas-praticas-ts-semana-ts-3/image-51.png)
...
![](./boas-praticas-ts-semana-ts-3/image-55.png)
...
![](./boas-praticas-ts-semana-ts-3/image-61.png)
...
![](./boas-praticas-ts-semana-ts-3/image-63.png)

Essa interface poderia ser escrita como uma extensão de outra interface:

![](./boas-praticas-ts-semana-ts-3/image-64.png)
...
![](./boas-praticas-ts-semana-ts-3/image-65.png)

E poderíamos deixar ela ainda mais genérica usando generics:

![](./boas-praticas-ts-semana-ts-3/image-66.png)
...
![](./boas-praticas-ts-semana-ts-3/image-68.png)
...
![](./boas-praticas-ts-semana-ts-3/image-69.png)
...
![](./boas-praticas-ts-semana-ts-3/image-70.png)
...
![](./boas-praticas-ts-semana-ts-3/image-72.png)
...
![](./boas-praticas-ts-semana-ts-3/image-73.png)
...
![](./boas-praticas-ts-semana-ts-3/image-74.png)
...
![](./boas-praticas-ts-semana-ts-3/image-76.png)
...
![](./boas-praticas-ts-semana-ts-3/image-75.png)

A não ser que a pessoa que esteja usando essa função passe o argumento de retorno, a saída não pode ser manipulada.

> Na verdade, essa é a forma que o `fetch` nativo do browser é implementado.
...
Os usos mais comuns do `never` são para indicar caminhos que uma função não pode percorrer, ou seja, formas que ela não pode ser usada.

> O `never` é bem mais comum em bibliotecas e outras tipagens que são estendidas por outras pessoas, raramente você vai ter que usar manualmente em um código de produção.
...
![](./boas-praticas-ts-semana-ts-3/image-78.png)
...
![](./boas-praticas-ts-semana-ts-3/image-79.png)
...
![](./boas-praticas-ts-semana-ts-3/image-80.png)

## Configuração

Além das boas práticas de código, existem também as boas práticas de configuração, ou seja, configurar o compilador do TS de forma que ele consiga dar pra você o melhor da inferência de tipos sem deixar que você caia em ciladas.

> Todas as opções que eu vou falar aqui são colocadas dentro da chave `compilerOptions` no seu `tsconfig.json`.

#### Sempre use `strict: true`

Como primeira recomendação, mantenha a opção `strict: true` ativa, isso vai garantir uma série de checagens de código explícitas que o compilador vai fazer por padrão, além de ser o modelo mais seguro e recomendado até mesmo pelos devs do TS.

> Ativar essa opção vai automaticamente ativar as opções seguintes e mais [várias outras](https://www.typescriptlang.org/tsconfig#strict)
...
![](./boas-praticas-ts-semana-ts-3/image-82.png)
...
![](./boas-praticas-ts-semana-ts-3/image-83.png)

Tenha em mente que essa é uma opção bastante severa, especialmente se você está migrando uma plataforma de JS para TS, dessa forma, a migração não vai ser só trocar os nomes dos arquivos, mas vai precisar de algumas mudanças manuais.

> Se você estiver a fim de usar o TS da forma como ele foi feito para ser usado, você pode ativar uma configuração [do ESLint chamada](https://github.com/typescript-eslint/typescript-eslint/blob/main/packages/eslint-plugin/docs/rules/no-explicit-any.md) `noExplicitAny`, que vai impedir que você use `any` em **qualquer lugar.**
