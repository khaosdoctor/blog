...
Um exemplo muito útil é a criação de um **contâiner de injeção de dependências**. Que é uma ferramenta muito útil para poder criar automaticamente instâncias de classes dependentes para classes que levam essas dependências, dessa forma você não precisa passar todas as dependências como parâmetros.<Sidenote>Isso já é uma realidade com a biblioteca [TSyringe](http://npm.im/tsyringe) feita pela Microsoft para demonstrar o poder dos decorators no TypeScript.</Sidenote>
