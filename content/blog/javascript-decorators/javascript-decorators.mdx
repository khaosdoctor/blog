...
A very useful example is building a **dependency injection container**, a very handy tool for automatically creating instances of dependent classes for classes that need those dependencies, so you don't have to pass every dependency as a parameter.<MarginNote>This is already a reality with the [TSyringe](http://npm.im/tsyringe) library, built by Microsoft to show off the power of decorators in TypeScript.</MarginNote>
