...
Durante a minha carreira eu já trabalhei com projetos pequenos e grandes usando JavaScript. A linguagem em si não é o problema, na verdade, muitos desses projetos só foram possíveis porque o JavaScript é muito mais simples do que outras linguagens, o maior problema de longe é a escalabilidade dessas aplicações, que é um reflexo direto de o JavaScript possuir uma _tipagem dinâmica_.[^n1]
...
Para ajudar ainda mais, podemos instalar toda a tipagem do Node.js, assim o TS vai saber tudo sobre o ambiente que a gente está rodando. A gente pode fazer isso com o comando `npm install -D @types/node`[^n2]
...
[^n1]: Eu já falei um pouco sobre tipagem estática e dinâmica na [minha série de artigos sobre Node.js por baixo dos panos](https://dev.to/_staticvoid/series/2080)

[^n2]: O `@types` é o escopo de uma organização chamada [DefinitelyTyped](https://github.com/definitelytyped) que possui tipos para vários pacotes famosos, nós vamos falar mais sobre ela nos próximos dias.
