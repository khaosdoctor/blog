...
During my career I've worked on small and large projects using JavaScript. The language itself isn't the problem, in fact, a lot of these projects were only possible because JavaScript is much simpler than other languages. The biggest problem by far is the scalability of these applications, which is a direct reflection of JavaScript having _dynamic typing_.[^n1]
...
To help even more, we can install all of Node.js's typings, so TS will know everything about the environment we're running in. We can do that with the command `npm install -D @types/node`[^n2]
...
[^n1]: I've already talked a bit about static and dynamic typing in [my article series about Node.js under the hood](https://dev.to/_staticvoid/series/2080)

[^n2]: `@types` is the scope of an organization called [DefinitelyTyped](https://github.com/definitelytyped) that holds types for lots of famous packages. We'll talk more about it in the coming days.
