...
Uma nota de rodapé para a fonte de uma afirmação, ou para um detalhe que interromperia a frase. Ela vira um link para o pé da página, com um link de volta. A marca é numerada, `[1]`, contra a letra que a `Sidenote` usa.
...
Uma `Sidenote` é marcada com uma letra e ancorada na palavra que ela segue. A contagem vai de `[a]` a `[z]` e continua em `[aa]`, como uma coluna de planilha, para não se confundir com o `[1]` das notas de rodapé. Aceita só conteúdo inline: texto, negrito, itálico, links e código.

```mdx
Alguma afirmação no texto.<Sidenote>Uma nota na margem.</Sidenote>
```

Toda abstração vaza.<Sidenote>Uma nota que fica na margem em telas largas e vira um popover em telas estreitas, sem JavaScript nos dois casos. Aceita **negrito**, `código` e [links](https://blog.lsantos.dev).</Sidenote> E esta segunda frase carrega outra.<Sidenote>Igual à de cima, uma letra adiante na contagem.</Sidenote>
