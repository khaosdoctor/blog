...
Uma nota de rodapé para a fonte de uma afirmação, ou para um detalhe que interromperia a frase. É markdown puro, então o Obsidian renderiza enquanto você escreve.
...
Uma afirmação que precisa de fonte.[^1] A mesma nota aparece de três jeitos, dependendo de onde você está lendo.[^n2] Em tela larga ela fica na margem, com a marca repetida no topo.[^n3] Em tela estreita a marca abre um painel na base da tela, e a lista completa continua no pé da página. Sem JavaScript em nenhum dos dois casos.
...
[^n2]: Aceita **negrito**, `código` e [links](https://blog.lsantos.dev), como qualquer nota de rodapé, porque é uma nota de rodapé.

[^n3]: A marca no topo desta caixa é a mesma que você acabou de passar na frase, então uma margem com várias notas continua dizendo qual responde o quê.
