...
epigraph: "Se você não consegue ver funcionando, você não sabe se funciona."
epigraphCite: "A regra desta página"
draft: false
noindex: true
---
...
O frontmatter é o cabeçalho YAML que define título, data, seção e tudo mais. Os campos `epigraph` e `epigraphCite` renderizam a citação que aparece acima do título desta página, antes do texto começar.
...
O resultado está no topo da página: o título, a linha de data e seção, e o epígrafe em itálico logo abaixo. O `noindex: true` mantém esta página fora dos mecanismos de busca, que é exatamente o que uma bancada precisa.
...
Uma citação em bloco para trechos de outra pessoa ou de outro documento. Para epígrafes existe um componente próprio, mostrado mais abaixo.
...
A mesma sintaxe do Obsidian, então o que você vê enquanto escreve é o que vai para o site. São cinco tipos: `NOTE`, `TIP`, `WARNING`, `CAUTION` e `IMPORTANT`.
...
## Epígrafe no meio do texto

Além do par de campos no frontmatter, que renderiza acima do título, existe o componente para colocar uma citação em qualquer ponto do corpo.

```mdx
<Epigraph cite="Quem disse">A citação.</Epigraph>
```

<Epigraph cite="Phil Karlton">Só existem dois problemas difíceis em computação: invalidação de cache e dar nome às coisas.</Epigraph>
