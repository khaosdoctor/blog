...
import counterSource from './components/Counter.vue?raw'
import counterHtml from './components/counter.html?raw'
...
As linhas de `import` no topo são a única exceção, e existem por causa das duas ilhas interativas do final: uma diretiva de cliente precisa de uma importação real, e o botão de ver o código precisa do arquivo como texto. Os dois arquivos moram em `components/`, dentro da pasta deste post. Nenhum post sem ilha importa nada, porque todos os outros componentes já são injetados em todos os posts.
...
O componente mora na pasta do próprio post, em `components/`, junto com as imagens. As duas linhas de `import` são o único lugar onde um `import` está certo em um post: uma diretiva de cliente precisa da importação real do componente, e a segunda traz o código-fonte como texto para o botão de ver o código. `client:visible` significa que o JavaScript do contador só é baixado quando ele entra na tela; o resto da página continua sendo HTML estático.

```mdx
import Counter from './components/Counter.vue'
import counterSource from './components/Counter.vue?raw'

<LabDemo source={counterSource} name="Counter.vue">
...
<HtmlLab html={counterHtml} name="counter.html" title="um contador em HTML puro" />
```
