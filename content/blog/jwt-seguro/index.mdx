...
O problema é que essa implementação é inerentemente **insegura**, e eu vou te dizer o por quê.[^n1]
...
Além disso a fingerprint é incluída no token, de forma que ela não possa ser alterada sem invalidar o token.[^n2]
...
[^n1]: O código usado nesse exemplo pode ser encontrado [no meu GitHub](https://github.com/khaosdoctor/secure-jwt-tokens)

[^n2]: O código para este exemplo está no branch `fingerprinting` do [repositório do GitHub](https://github.com/khaosdoctor/secure-jwt-tokens). Veja o arquivo `handlers.ts` e `index.js` para as [diferenças](https://github.com/khaosdoctor/secure-jwt-tokens/compare/fingerprinting)
