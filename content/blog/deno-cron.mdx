...
Por enquanto a [documentação do cron](https://docs.deno.com/kv/manual/cron) diz que só é possível criar cronjobs no top-level domain. Ou seja, **não é possível criar um cron dentro de uma função** ou dentro de algum escopo, dessa forma é necessário que você já saiba o que precisa ser executado de antemão. Para criar cronjobs dinâmicos as [filas](blog.lsantos.dev/deno-kv-queues/) podem ser uma boa opção!
