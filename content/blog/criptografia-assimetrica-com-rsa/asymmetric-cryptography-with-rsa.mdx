...
Remember: asymmetric cryptography is a **cryptographic system** (_cryptosystem_) that gets implemented by several algorithms. **One of them** is RSA, but it's not the only one, so there are several different implementations of private keys, but they'll all share the same properties.

Private keys are used to **decrypt** data encrypted by the public key, or to **sign** data that can later be validated by the public key. **It's not advisable to encrypt data using the private key**, because any public key can decrypt that data. That's exactly why these keys are called **signers** (we'll get into the concept of signatures in the other articles).
...
We take the DEM block, split it into key and file, decrypt the symmetric key using the private key, get back the original symmetric key, and use it to decrypt the encrypted file.

> The HTTPS protocol works "roughly" like this.
...
-   Any key above 3072 bits is considered secure

Remember that when we talk about keys here, we're talking about numbers, nothing more. It's like a password, except it has 1234 digits (`2^4096`). When we're "encrypting" something, we're applying an exponentiation operation between the key and the value we want to encrypt.
...
-   `n` is `p*q`, which is `41*97=3977`. Now we have the first value you saw back in step 3 above (try factoring 3977 to get back to its primes, how many steps did it take?)

Remember that `p` and `q` are private and can't be shared, while `n` is public.
