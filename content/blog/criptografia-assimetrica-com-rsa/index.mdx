...
Pegamos o DEM block, separamos em chave e arquivo, decriptamos a chave simétrica usando a chave privada, obtendo a chave simétrica original, que podemos usar para decriptar o arquivo encriptado.[^n2]
...
-   Qualquer chave acima de 3072 bits é considerada segura[^n3]
...
-   `n` é `p*q` que é `41*97=3977`, agora temos o primeiro valor que você viu no passo 3 lá em cima (tente fatorar 3977 para chegar nos primos dele, quantos passos levou?)[^n4]
...
[^n1]: Criptografia assimétrica é um **sistema criptográfico** (_cryptosystem_) que é implementado por vários algoritmos. **Um deles** é o RSA, mas ele não é o único, portanto existem várias implementações de chaves privadas diferentes, mas todas vão ter as mesmas propriedades.

[^n2]: O protocolo HTTPS funciona "mais ou menos" assim.

[^n3]: Quando falamos de chaves aqui, estamos falando de números, nada mais. É como se fosse uma senha, só que ela tivesse 1234 dígitos (`2^4096`).

[^n4]: Lembrando que `p` e `q` são privados, não podem ser divulgados, enquanto `n` é público.
