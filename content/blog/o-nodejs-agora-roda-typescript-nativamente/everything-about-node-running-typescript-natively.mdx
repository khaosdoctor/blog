...
But now that's no longer necessary! Starting with [Node 22.6](https://nodejs.org/en/blog/release/v22.6.0), two new experimental flags were added:[^n1]
...
[^n1]: The funny thing here is that this flag's name was supposed to be `enable-transformation`, and I [suggested](https://github.com/nodejs/node/pull/54283#discussion_r1711462259) we change it to something like `enable-type-transformation` to keep the semantics right.

[^n2]: You can also check out the [Node docs article](https://nodejs.org/api/typescript.html) about this change.
