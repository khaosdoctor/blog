...
> [!TIP]
> You can check some of my other tutorials [here](https://www.harperdb.io/author/lucas-santos) or [in this particular community article](https://www.harperdb.io/development/tutorials/harperdb-authentication-with-oauth) to know how to set up your infrastructure locally, with Kubernetes, or in the cloud.
...
To use TypeScript you need Node.js installed, be sure to use the latest LTS version. You can check it by running `node -v` in your terminal. If you don't have it installed, you can download it [here](https://nodejs.org/en/download/), or use a version manager like [asdf](https://asdf-vm.com/#/core-manage-asdf-vm), [nvm](https://github.com/nvm-sh/nvm), or even [volta](https://volta.sh/).<Sidenote>I'm using version 20.7.0 of Node.js, but you can use any version above 18 at the time of writing this article.</Sidenote>
...
> [!TIP]
> You can check the latest version of TypeScript [here](https://www.npmjs.com/package/typescript). I'm using version 5.2.2
...
> [!CAUTION]
> Never commit secrets to your repository. Use a `.env` file to store them and add it to your `.gitignore` file.
...
> [!TIP]
> You can check the [Node.js documentation](https://nodejs.org/api/crypto.html#crypto_crypto_randomuuid_options) to know more about the `randomUUID` function.
...
> [!TIP]
> One other thing that we can do here is to not use the `toObject` method and instead use the shorthand `{ ...item }`, which will automatically convert it into an object. But I prefer to have a method that I can call to make it more explicit. You can also return `{ ...this }`
...
> [!TIP]
> It's a best practice to name external APIs as 'clients', if we had any other data layer, let's say a queue system that's not connected through an API, we could just name it `queueAdapter` or `queue` and it would be fine. This is not a rule, but I personally think it's easier to understand what the file is doing, if an external agent or an internal driver.

Our client is an HTTP API, so let's use the native `fetch` from node to communicate with it.<Sidenote>The fetch api is only available in node from version 18 and up.</Sidenote> Let's add the following code to our `TodoItemClient.ts` file:
...
> [!TIP]
> You can check the [HarperDB documentation](https://api.harperdb.io/#25977fef-53e8-40bf-a26a-43369bc3721c) to know more about the operations.
...
> [!NOTE]
> In any case, we have already implemented another level of validations in our Domain object when we are receiving objects because TypeScript will only enforce them at the compile time, so we can be sure that the data is valid.
...
> [!NOTE]
> We could have created another file called `config.ts` and moved both our `EnvironmentSchema` and `conf` object there, but since we are only using it in the `index.ts` file, I prefer to keep it there. However, if you receive that configuration somewhere else, you should create a `config.ts` file and move it there.
