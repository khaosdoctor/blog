...
category: technology
tags: ["typescript", "harperdb", "databases", "nodejs", "architecture", "rest"]
...
To use TypeScript you need Node.js installed, be sure to use the latest LTS version. You can check it by running `node -v` in your terminal. If you don't have it installed, you can download it [here](https://nodejs.org/en/download/), or use a version manager like [asdf](https://asdf-vm.com/#/core-manage-asdf-vm), [nvm](https://github.com/nvm-sh/nvm), or even [volta](https://volta.sh/).[^n1]
...
Our client is an HTTP API, so let's use the native `fetch` from node to communicate with it.[^n2] Let's add the following code to our `TodoItemClient.ts` file:
...
[^n1]: I'm using version 20.7.0 of Node.js, but you can use any version above 18 at the time of writing this article.

[^n2]: The fetch api is only available in node from version 18 and up.
