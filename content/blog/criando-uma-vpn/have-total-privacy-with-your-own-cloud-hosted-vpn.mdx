...
Let's log in to the server to make the changes, for that just use the command:<Sidenote>Remember that the user is the same user you set when you created the server, and the IP is the public IP address that shows up as a link on your VM panel in Azure.</Sidenote>
...
To do that let's open the OpenVPN config file with:<Sidenote>Remember `vim` can be any editor.</Sidenote>
