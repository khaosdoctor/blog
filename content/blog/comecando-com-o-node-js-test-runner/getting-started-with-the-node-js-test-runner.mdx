...
To check if everything's fine, just run `node --test` in an empty directory.<MarginNote>The command is recursive, so it'll try to go into every directory below it.</MarginNote> Since Node won't find any files, your output should look like this:
