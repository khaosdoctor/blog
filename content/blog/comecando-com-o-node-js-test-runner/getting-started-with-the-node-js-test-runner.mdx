...
To check if everything's fine, just run `node --test` in an empty directory.[^n1] Since Node won't find any files, your output should look like this:
...
If you run the command now, nothing will change, because our mjs file is still the same and TSX can run it just fine. Now let's change the extensions to `.mts` and run the test again with `npm t`. Your output will look something like this:[^n2]
...
[^n1]: The command is recursive, so it'll try to go into every directory below it.

[^n2]: The error we're seeing on code coverage is the reason it's still marked `--experimental`. It's already been reported [here](https://github.com/nodejs/node/issues/52775) and there seems to be a fix [here](https://github.com/nodejs/node/pull/53153).
