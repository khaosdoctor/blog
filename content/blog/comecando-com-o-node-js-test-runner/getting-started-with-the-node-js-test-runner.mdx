...
To check if everything's fine, just run `node --test` in an empty directory.<Sidenote>The command is recursive, so it'll try to go into every directory below it.</Sidenote> Since Node won't find any files, your output should look like this:
