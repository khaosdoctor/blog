...
Vamos adicionar um novo usuário chamado Alice a este grupo que acabamos de criar, vamos então copiar o ObjectID quando criarmos:<MarginNote>Quando criarmos um usuário precisamos de um domínio **válido**. Para obter este domínio podemos executar `az ad user list --query "[*].userPrincipalName" -o json` e copiar tudo o que vier depois do "@".</MarginNote>
...
Vamos criar agora o binding deste role ao nosso ID do grupo no AD:<Sidenote>Podemos também vincular o role a um usuário utilizando o ObjectID do usuário ao invés do ObjectID do grupo no comando acima.</Sidenote>
