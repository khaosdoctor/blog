...
Let's add a new user called Alice to the group we just created, and copy her ObjectID as we create her:[^n1]
...
Now let's create the binding of this role to our AD group's ID:[^n2]
...
[^n1]: When creating a user we need a **valid** domain. To get this domain, run `az ad user list --query "[*].userPrincipalName" -o json` and copy everything that comes after the "@".

[^n2]: We can also bind the role to a single user by using the user's ObjectID instead of the group's ObjectID in the command above.
