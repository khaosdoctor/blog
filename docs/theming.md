...
**7. Palette lab (`PaletteLab.vue`).** Seven registers: the brand as it is, canonical CGA, VileR's measured 5153 CGA, P39 phosphor, amber, Game Boy DMG, and Campbell (the Windows Terminal default). Every swatch renders as actual text with its hex, its live contrast ratio and its WCAG grade. Knobs: background lightness, background tint (neutral, cool, warm, green), a "lighten the colours" mix towards white, and a toggle to measure against the large-text threshold of 3:1 instead of 4.5:1. A counter says how many colours in the current register fail.
