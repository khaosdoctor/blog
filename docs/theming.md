...
The visual direction in full. `docs/design.md` was the short companion to this file and has been removed, so the
names of both it and `docs/decisions.md` appear below only as records of what was true when each section was
written. This file records what was looked at, what was tried, what was thrown away and why, and where the reasoning is still incomplete. Every candidate named here exists and is interactive at `/theme-lab-arquivo/`: `/theme-lab/` closed once its last open section was decided, and every bench it ever held, decided or not, lives there now. The rest of this file still describes them by their old `/theme-lab/` section numbers, a snapshot of where things stood while the lab was still open, not a live address.

This started as a pure exploration with nothing applied to the site, and that is no longer true. The parts that have since been adopted are marked as decided where they appear, and the short list of them is: the two page grounds, the full palette with a tone per colour per ground, the display and subtitle faces, and no CRT scanline effect. Everything else here is still a candidate.
...
Nine Vue islands in `content/blog/theme-lab/components/`, plus five shared files. The count keeps moving: it was eleven, then twelve with the cover candidates, then thirteen with the logo, and dropped to nine once four candidates retired to the archive (section 7 has the full accounting):
...
- `DecisionCopy.vue`. One button, now wired into every demo, live and archived alike (the archived ones import it from the live lab rather than carrying their own copy). It copies a self-contained prompt to the clipboard: the candidate's name, its component file, every knob's resolved label and value, and the measured contrast for whatever is on screen, plus a line telling whoever pastes it to archive the candidates that lost rather than delete them. It exists because describing a decision from memory kept losing the exact numbers.
...
The live page is at `/theme-lab/`, renumbered since four candidates retired: section 00 Tipografia, 01 Cor, 02 Interface, 03 A capa, 04 O logo animado. What follows is what each one is, what its knobs do, and what it costs, in that order. The four that lost are not here: they moved to `/theme-lab-arquivo/`, and get their own accounting after this list, because that is where they now live and because the reasons they lost are worth reading together rather than interleaved with what is still open.

**1. Type specimen (`TypeSpecimen.vue`), section 00.** Now that display and subtitle are decided, this demo picks up where section 6 above leaves off: the title renders in Departure Mono and the deck in PxPlus IBM VGA8, both fixed and labelled as already decided, and the one picker left chooses the body face, from the seven candidates still in play (IBM Plex Mono, Handjet, Inter, Roboto, Source Serif 4, Literata, Atkinson Hyperlegible). Four real paragraphs, a diacritics line, a code sample, four backgrounds (site dark, site light, phosphor, amber). Sliders for size, line height, letter spacing, word spacing and measure, with a live WCAG 1.4.12 verdict. Shows the chosen face's licence, role and width.

**2. Whole-pixel ladder (`WholePixel.vue`), section 00.** Fifteen sizes from 8 to 32px, on-grid ones marked. Integer `zoom` magnifier, anti-aliasing kill switch, dark/light background.

**A post of real text, section 00, no component.** No candidate number and no knobs: a full post body written as plain markdown, run through the same remark and rehype pipeline as any published post. The live page is explicit about what this is: the ruler the two candidates above are measured against, kept apart from them on purpose. It exists because judging a body face against a specimen's isolated paragraphs hides what a real post actually asks of it. The page covers bold, italic, strikethrough and underline (the one emphasis with no markdown syntax of its own, written as a raw `<u>` tag rather than passing through remark like the rest); inline code and two fenced code blocks, one with a filename comment that becomes its tab and one without; a bare blockquote next to a `[!quote]` callout with an author, so both quote treatments render side by side; all seven callout types (`NOTE`, `TIP`, `WARNING`, `CAUTION`, `IMPORTANT`, `QUESTION`, `EXAMPLE`); a footnote, a sidenote and a margin note, through the same `Sidenote.astro` and `MarginNote.astro` used on real posts; both list kinds, including a task list; a horizontal rule; and an image whose caption comes from the markdown title rather than its alt text, backed by a `placeholder.png` generated locally with `sharp` at the real 1200×630.

**3. Palette lab (`PaletteLab.vue`), section 01.** Eight registers: the brand as it is, the site's own ink roles (`--fg`, `--muted`, `--accent` and `--rule`, light and dark, plus the brand purple), canonical CGA, VileR's measured 5153 CGA, P39 phosphor, amber, Game Boy DMG, and Campbell (the Windows Terminal default). Every swatch renders as actual text with its hex, its live contrast ratio and its WCAG grade. The background picker's first two options are the site's real grounds, `#000000` and `#f4efe0`, not an approximation of either, and dark is the default; choosing one of the two hides the background-lightness knob, since it has nothing left to adjust. Remaining knobs: background tint (neutral, cool, warm, green) for the other four backgrounds, a "lighten the colours" mix towards white, and a toggle to measure against the large-text threshold of 3:1 instead of 4.5:1. A counter says how many colours in the current register fail.

**4. CRT effects (`CrtEffects.vue`), section 01.** Scanlines with a separate pitch control, glow, grain, vignette, flicker. **Everything starts at zero.** The readout shows the base contrast, the contrast on the dark scanline row, and the difference, recomputed as you drag. There is no curvature and there will not be. The grain is an inline `feTurbulence` SVG rather than a `data:` URI, which keeps it clear of the CSP with nothing to configure.

**5, 6, 7. Chrome (`ChromeHeader.vue`, `ChromeList.vue`, `ChromeButton.vue`), section 02.** Five headers (box-drawing bar, inverted DOS status line, minimal dotted rule, Game Boy menu with a `▸` cursor, dense ledger with `SEÇÃO 00 / ÍNDICE` and a version string), four post lists (dense four-column table, ledger with dotted leaders, Game Boy menu, cards with the site's existing thick-left-edge idiom), five buttons (brackets, solid block, double frame, prompt prefix, menu cursor on hover). All character-only: no image, no icon, no request. Every one carries face, tracking, density and colour knobs, and the button demo reports the touch target height against the 44px minimum.

**8. Cover candidates (`CoverLab.vue`), section 03.** This is the actual OG/social card, 1200×630, that ships in `og:image`. Three real `<svg viewBox="0 0 1200 630">` elements, not canvas, because the real generator rasterises that same markup with `sharp` at build time, so what renders here matches exactly what the build will output. The first is a DOS window: black background, a double rectangle frame with the space between the two lines widened after an earlier pass came back too tight to read as a frame rather than a doubled line. The second drops the frame and floods the card in the brand colour at full strength, with a rule at 75% of the card's width under the kicker, and the ink changes to a dark colour only for the one brand hue, yellow, where light text on light would fail. The third is a plasma field from three summed sine waves, unique per post, with the title carrying a hard 3px shadow because the field underneath it can turn light enough to erase plain text. Nothing here calls `Math.random()`: a small string hash of the post's slug picks both the brand colour and the plasma's phase, so a rebuild of an existing post never changes its cover; the lab's own seed knob only adds to that hash so the five brand colours can be cycled through without touching code. Each candidate prints its own live title contrast, and the plasma's is marked as a worst case, since the background underneath it is the one card here that is not a flat colour.

**9. Logo animation (`LogoLab.vue`), section 04.** The decision that opens the whole live page now: the site carries almost no animation, and the header mark is the one exception, moving low and constant, the way a disk-activity light does, so the page reads as alive without competing with the text for attention. Five candidates, all colouring or clipping the same five rectangles the favicon is drawn from (a stand-in for the real mark vector, which was not reachable from this machine, copied shape for shape from `public/favicon.svg`): colour cycling through the accents, a sliding brightness band, a character-ramp scramble on an 8x8 rasterised version of the mark, a sequential trace across the accents, and a pulse on the accents. Next to the mark, the wordmark "Lucas Santos" is shown in full on load and, after a delay, scrambles letter by letter into a small block-character ramp before collapsing into the mark rather than simply vanishing, the same idea as textmode.js's own scramble effect, reimplemented here with a plain `setInterval` and a CSS transition instead of the library. The accessible name lives in `aria-label` on the link, not in the text that disappears, so it survives regardless of which candidate or animation state is showing. `prefers-reduced-motion` freezes every candidate and stops the wordmark scramble outright rather than pausing it partway, and a manual pause toggle covers what WCAG 2.2.2 asks of anything that moves for more than five seconds.

---

### The four that retired

`TmHeading.vue`, `TmCover.vue`, `TmSolid.vue` and `CssHeading.vue` lost their decisions and moved to `content/blog/theme-lab-arquivo/`, a `noindex` post, where they still render with the same knobs, importing the live lab's shared controls rather than duplicating them. Nothing was deleted, on the same reasoning as section 5 above: the rejected options are half of the argument for the article the owner intends to write about how this redesign was decided, so `docs/decisions.md` and this file are not the only record of them.

**`TmHeading.vue` and `TmSolid.vue` lost to the same decision, and it is the one above them both: the site keeps almost no animation, and the only place that keeps constant movement is the logo.** `TmHeading` was the first candidate of all and the closest to the screenshot that started this work: a post title over a WebGL2 character field, four movements (concentric wave, column rain, Perlin noise, crossed grid), five character ramps, cell size, speed, density, freeze, and colour across the four brand colours plus phosphor and amber. A field behind every post title is the opposite of "almost no animation," so it lost outright; it stays archived as a candidate for a single page, something like an about page, that might still want a field like this. `TmSolid` was a rotating torus, box, sphere, cone or cylinder resolved into characters by the library's 3D pipeline, meant for a 404 or a footer ornament; it lost for the same reason and was also the most expensive of the three animated candidates, a WebGL2 loop that never stops on its own. Two lessons from building it are worth keeping regardless of the verdict: `frameCount` is a getter in the library, and calling it as a function crashed the whole scene, and without an explicit `camera()` call the library starts with the eye inside the torus, which rendered the tube's inner wall as a green field with a black hole in the middle.

**`CssHeading.vue` lost because it reproduced textmode.art's own page rather than this blog's.** It is the exact effect from the screenshot that started this work, the typed line and block cursor, with no WebGL at all: a kicker, a heading with an accent word, a terminal box with a `>_` prompt, a bracketed button, six faces, five accents including the reference's own periwinkle `#a8b1ff`, four border treatments. It is kept archived on purpose, and it is the most important of the four to keep, because it is proof that the screenshot effect never needed the WebGL library: the typed line is a Vue component of about forty lines with no canvas in it, sitting right next to the candidate that spends 194KB to reach a similar result. The brackets around its button label are `::before`/`::after`, so the button stays a real, focusable button and a screen reader never announces the brackets as content, a detail worth reusing for any button on the site regardless of this candidate's fate.

**`TmCover.vue` lost because it was the same decision twice.** It used the same library to paint cell backgrounds (`cellColor`, the ANSI shade-dither technique from section 3) and draw a `╔═╗` character frame around a still cover image, with four patterns, six palettes (not seven; the brand's four colours plus phosphor and amber), cell size, seed, an art ceiling capping how close the shading gets to full brightness, and a title-length selector at 23, 58 and 90 characters. It lost because candidate 8 above, `CoverLab.vue`, already does this job in real `<svg>` at the same 1200×630, and SVG is exactly what `sharp` rasterises at build time, so keeping a second, canvas-based cover bench only split attention between two answers to the same question, and the SVG one is what becomes production code. Three lessons cost real time building it and are worth keeping on record: a `<canvas>` with no explicit size defaults to 300x150, and since the library draws to whatever canvas it is given rather than one it creates itself, the first frame was born at that size and appeared printed small in the corner; the library's own splash screen, if the animation loop stopped mid-fade, left the splash frame's border printed over the cover permanently; and a black box added behind the title so it would not disappear into the art, once measured, put one palette's ink (the `amarelo cheio` palette's `#332d23`, chosen for the brand yellow it is normally paired with) at 1.54:1 against that box's `#000000`, worse than the problem the box was meant to fix. All three are the same mistake: assuming what is behind whatever is being drawn.

**No candidate left on the live page uses textmode.js.** Everything still in play, sections 00 through 04, is SVG, CSS and DOM. The library stays installed as a `package.json` dependency for one reason only: the four archived candidates above still import it. Section 1 opened this entire file with the textmode.js brief; this is the honest outcome of that brief, and it belongs in the recommendation below as much as here.
