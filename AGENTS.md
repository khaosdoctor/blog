...
**Posts are `.mdx` files containing plain markdown.** Never an import. `remark-embeds`, `remark-figures` and `remark-lab-demos` turn ordinary markdown into components at build time, and the component set is injected into every post, so the handful of tags markdown has no syntax for (`<Video>`, `<LabDemo>`, `<HtmlLab>`) are written bare. This is deliberate: `content/` is an Obsidian vault, Obsidian renders almost none of that, and writing in Obsidian is the point of the whole rebuild. Do not "modernise" the rest of a post back into components.
...
**Some logic is deliberately duplicated and must be mirrored by hand.** `BaseLayout`'s blocking pre-paint script shares no source with the client modules it pre-empts, so a storage key name, a clamp bound or an applied-value rule edited on one side has to be edited on the other or the page flashes. The same applies to: the settings panel's Conway slider bounds against `src/lib/tweaks.ts`; the `hp-persist` key, where `'0'` means off and anything else including absence means on; the `hp-footnotes` key, where `'1'` means on and absence means off; and any code deciding whether to link to a series index, which must use the same "more than one published part in this language" condition the route builds on, or the link 404s.

**The languages share one implementation.** Astro's file routing needs a route file per language, so `src/pages/en/` mirrors the root, but each file is a delegation naming its own locale: the page body lives in `src/components/pages/`, the path list in `src/lib/routes.ts`. Nothing outside `src/i18n/` may name a language, spell out a `/en/` prefix, or hold a two-entry pt/en table; `LOCALES`, `SOURCE_LOCALE` and `localePath()` come from `src/i18n/locales.ts`, which imports nothing so `astro.config.mjs`, the remark plugins and the node scripts can all reach it. `scripts/check-i18n.ts` warns on a restated locale pair.
...
**The build fetches only its own media.** `prebuild` runs `scripts/vendor-media.ts`, which downloads any remote image a post references into the post directory and rewrites the reference, so the site stops depending on other people's servers, then `scripts/fetch-link-titles.ts`, which reads the `<title>` behind every external link into `content/link-titles.json` so a hover preview can name the page it points at. Neither ever fails the build, and both are no-ops once their file is written. A link whose title never arrives is stored as `null` and the preview falls back to the link's own text from the post, so a blocked host costs nothing. Tweet and bookmark metadata come from `content/bookmarks.json`, written by hand and preferred over anything crawled.
...
**Markup works without JavaScript; a script only upgrades it.** A control that has somewhere to go is a real `<a href>` and stays one, so it works before the script runs and with scripting off. The script takes the click over with `preventDefault()`. What breaks by accident: `aria-haspopup`, `aria-controls` and anything else describing script-only behaviour is added BY the script, never written in the markup, or the element claims a capability it does not yet have; and a visible promise of a keyboard shortcut stays invisible (its space reserved) until the listener behind it is live. A control that genuinely cannot work without JS starts `hidden` and its script unhides it, so nothing dead is ever on screen.

**A lab demo's source is never in the post.** `<LabDemo>` and `<HtmlLab>` get their props from `remark-lab-demos`, which emits a link to `/lab-source/<post>/<path>.html`, one page per file under the post's `components/`, built by the `labSource` collection. `src/scripts/lab-source.ts` fetches that page on click and inserts the block; with scripting off the link opens the page instead. Never put the file back in the tag as a slot: the archive post carries a whole folder of demos, and highlighting all of them into one page is what made it many megabytes and ran the dev server out of heap. The pages are `noindex` and `data-pagefind-ignore`, since Pagefind crawls `dist/` and would otherwise index every source file as a document.

**TypeScript, and `.mjs` only when something genuinely will not take a `.ts`.** New code is `.ts`, or `.astro` with a typed frontmatter block. If you reach for `.mjs`, say in one line what refused the `.ts`, because "it was quicker" is not that.

Fifteen files predate this and are converted as they are touched: `astro.config.mjs`, `src/integrations/redirect-stubs.mjs` and everything in `src/plugins/`. Renaming one is not a rename. They walk hast and mdast trees with no annotations, so converting the set at once produced 213 type errors, 191 of them in the plugins, almost all a parameter with no type. Convert one while you are already changing it and type `node`, `tree` and `file` against `@types/hast` and `@types/mdast`, both already installed. `public/sw.js` stays `.js`: it is served as a static asset, not built.
